...
def fetch_cloudinary_images(creds: dict[str, str]) -> list[dict[str, Any]]:
    ...
    if folder:
        ...
        print(f"📡 Menghubungi Cloudinary [{cloud_name}] (root storage)...")
    ...
    print(f"📥 Ditemukan {len(resources)} file foto di Cloudinary.")
    return resources
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    print(f"  ⬇️ [DOWNLOAD] Employee: '{emp_id}' <- {local_file.name}")
        except Exception as e:
            print(f"  ⚠️ Gagal download {secure_url}: {e}")
...
def run_c_enrollment(enroll_dir: Path, bin_path: Path) -> None:
    ...
    if not exe_path.exists():
        print(f"\nℹ️ Binary '{exe_name}' belum di-build.")
        print("   Jalankan 'build_and_test.bat' terlebih dahulu untuk mengompilasi.")
        return

    print(f"\n🧠 Menjalankan Native C Enrollment Engine ({exe_name})...")
    for emp_dir in sorted(enroll_dir.iterdir()):
        ...
        try:
            ...
            if res.returncode == 0:
                print(f"  ✅ [C ENROLLED] '{emp_id}'")
            else:
                print(f"  ⚠️ [WARNING] '{emp_id}': {res.stderr.strip() or res.stdout.strip()}")
        except Exception as err:
            print(f"  ❌ Error running {exe_name} for '{emp_id}': {err}")
...
def main() -> None:
    ...
    if not creds:
        print("❌ Kredensial Cloudinary belum diatur di .env!")
    ...
    try:
        ...
        print(f"📊 Ringkasan: {downloaded} foto baru diunduh, {skipped} foto sudah ada.")
        ...
        if has_insightface and has_cv2:
            print("\n🧠 Menjalankan model InsightFace Python untuk ekstraksi embedding...")
            ...
            for emp_dir in sorted(target_enroll.iterdir()):
                ...
                if emp_vectors:
                    ...
                    print(f"  ✅ [ENROLLED] '{emp_dir.name}' ({len(emp_vectors)} foto valid)")
            ...
            if emp_ids:
                ...
                with open(target_bin, "wb") as f:
                    ...
                    for i in range(len(emp_ids)):
                        ...
                        f.write(vec.tobytes())
                print(f"\n💾 [C Binary Ready] Berhasil membuat '{target_bin}' ({len(emp_ids)} karyawan).")
        else:
